...
Relay_state_t Relay_state = STATE_OFF, Relay_Oldstate = STATE_ON;
uint8_t RelayindMode = 0;

uint32_t temp32;
float tempFloat, Relay_OldDC;
...
void Module_Init(void) {
	...
	xTaskCreate(ThermocoupleTask, (const char* ) "ThermocoupleTask",
			(2*configMINIMAL_STACK_SIZE), NULL,
			osPriorityNormal - osPriorityIdle, &ThermocoupleHandle);
...
Module_Status Module_MessagingTask(uint16_t code, uint8_t port, uint8_t src,
	...
	case (CODE_H09R0_STREAM_PORT_C):
		...
		StreamCToPort(cMessage[port - 1][9 + shift],
	...
	case (CODE_H09R0_STREAM_PORT_K):
		...
		timeout = ((uint32_t) cMessage[port - 1][5 + shift] << 24)
				...
				+ ((uint32_t) cMessage[port - 1][7 + shift] << 8)
				+ cMessage[port - 1][8 + shift];
		StreamKToPort(cMessage[port - 1][9 + shift],
				cMessage[port - 1][10 + shift], period, timeout);
		break;
	...
	case (CODE_H09R0_STREAM_PORT_F):
		...
		timeout = ((uint32_t) cMessage[port - 1][5 + shift] << 24)
				...
				+ ((uint32_t) cMessage[port - 1][7 + shift] << 8)
				+ cMessage[port - 1][8 + shift];
		StreamFToPort(cMessage[port - 1][9 + shift],
				cMessage[port - 1][10 + shift], period, timeout);
		break;
	...
	case (CODE_H09R0_SAMPLE_C):
		SampleC(&H09R0_Temp_C);
		break;
	case (CODE_H09R0_SAMPLE_K):

		SampleK(&H09R0_Temp_K);
		break;
	case (CODE_H09R0_SAMPLE_F):
...
void ThermocoupleTask(void *argument) {
	...
	while (1) {
		...
		case STREAM_BUFFER_CASE:
			t0 = HAL_GetTick();
			SendResults(DATA_To_SEND, global_mode, unit, 0, 0,
					ptr_thermo_buffer);
			while (HAL_GetTick() - t0 < global_period) {
				taskYIELD();
			}
